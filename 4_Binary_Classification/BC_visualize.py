...
def draw_3D(model,X,y):
    model.eval()
    with torch.inference_mode():
        plt.figure(figsize=(5, 5))
        ax = plt.axes(projection='3d')
        ax.scatter(X[:, 0], X[:, 1], y, s=1, c=y)
        step1 = 0
        step2 = 0
        for i in range(0,195):
            if i<100:
                x_limit = np.linspace(-1.2,1.2, 100)
                y_limit = np.linspace(-1.2+step1,1.2-step1, 100)
                step1 = step1 + 0.025
                xy_limmit = np.stack((x_limit, y_limit), axis=-1)
                ax.plot3D(x_limit, y_limit, model(torch.tensor(xy_limmit).float()).ravel(), c='green',linewidth=1)
            else:
                x_limit = np.linspace(-1.2+step2,1.2-step2, 100)
                y_limit = np.linspace(-1.2,1.2, 100)
                step2 = step2 + 0.025
                xy_limit = np.stack((x_limit, y_limit), axis=-1)
                ax.plot3D(x_limit, y_limit, model(torch.tensor(xy_limit).float()).ravel(), c='green',linewidth=1)
...
def plot_decision_boundary(model: torch.nn.Module, X: torch.Tensor, y: torch.Tensor):
    ...
    plt.contourf(xx, yy, y_label, alpha=0.4)
...
#load data:
X,y = make_circles(n_samples=N_SAMPLES,shuffle=True,noise=NOISE,random_state=seed,factor=FACTOR)
