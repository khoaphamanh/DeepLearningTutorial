...
def hyperparameter_set(params_dict:dict,num_model:int):
    set = []
    for i in range(num_model):
        for j in range(len(params_dict)):
            tmp = []
            tmp.append(random.choice(params_dict['hidden units']))
            tmp.append(random.choice(params_dict['n_layers'])),
            tmp.append(random.choice(params_dict['learning_rate']))
            tmp.append(random.choice(params_dict['epoch']))
        set.append(tmp)
    return np.array(set)
...
def train_test_model(X_train,X_val,y_train,y_val,hidden,n_layers,learning_rate,epoch):
    ...
    for i in range(epoch):
        ...
        with torch.inference_mode():
            ...
            # calculate the loss and accuracy
            loss_test = loss(y_pred_test_probability, y_val)
            accuracy_test = accuracy_function(y_pred_test_label, y_val)

    return float(accuracy_test)

def random_search_CV(X_train,X_val,y_train,y_val,parameter_dict):
    torch.manual_seed(SEED)
    params_combi_all_result = []
    index = 0
    for params_combi in hyperparameter_set(params_dict=parameter_dict,num_model=NUM_MODEL):
...
#hyperparameter
hyperparameter = {
    'hidden units':[x for x in range(2,32,2)],
    'n_layers': [x for x in range(0,5,1)],
    'learning_rate': [x for x in np.arange(0.05,0.5,0.05)],
    'epoch':[x for x in range(50,300,50)]
}

#hyperparameter_combination
hyperparameter_combination = hyperparameter_set(hyperparameter,NUM_MODEL)

#hyperparameters tuning with gridsearch cross validation
test = random_search_CV(X_train,X_val,y_train,y_val,hyperparameter)
print("result of model with all hyperparameters combination ",test)
print("the best model is model number {} with mean accuracy test {}".format(test.index(max(test)),max(test)))
...
print("final testing is: accuracy train = {}, accuracy test = {}".format(final_testing[0],final_testing[1]))